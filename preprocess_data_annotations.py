...
from utils.transform_suffixes_new import SuffixTransform, apply_suffixtransform
from utils.helpers import write_lines,read_lines,get_verb_form_dicts, read_parallel_lines, encode_verb_form, \
    apply_reverse_transformation, SEQ_DELIMETERS, START_TOKEN
from utils.convert_parallel_to_tags import par_to_edit

nlp = spacy.load("en")

def perfect_align(t, T, verb_form_dict, insertions_allowed=0,
            ...
            for q in range(insertions_allowed + 1):  # Go inclusive
                if i < len(t):
                    ...
                    for k in range(j, len(T) + 1):
                        transform = \
                            apply_transformation(t[i], '   '.join(T[j:k]),verb_form_dict)
                        if transform:
                            cost = 0
                        else:
...
def align_sequences(source_sent, target_sent,verb_form_dict):
    # check if sent is OK
    ...
    for diff in diffs:
        ...
        if tag == 'equal':
            ...
            _, alignments = perfect_align(source_part, target_part,verb_form_dict,
                                          insertions_allowed=0)
            for alignment in alignments:
                new_shift = alignment[2][0]
                edits = convert_alignments_into_edits(alignment,
                                                      shift_idx=i1 + new_shift,verb_form_dict=verb_form_dict)
                all_edits.extend(edits)
...
def convert_alignments_into_edits(alignment, shift_idx,verb_form_dict):
    edits = []
    ...
    for i in range(1, len(target_tokens)):
        ...
        transform = apply_transformation(source_token, target_token,verb_form_dict)
        if transform:
...
def convert_data_from_raw_files(source_file, target_file, output_file, chunk_size,  verb_form_dict):
    tagged = []
    ann_tagged = []
    ...
    for source_sent, target_sent in tqdm(zip(source_data, target_data)):
        try:
            aligned_sent = align_sequences(source_sent, target_sent, verb_form_dict)
        ...
        try:
            check_sent = convert_tagged_line(aligned_sent, verb_form_dict)
        except Exception:
            # debug mode
            aligned_sent = align_sequences(source_sent, target_sent, verb_form_dict)
            check_sent = convert_tagged_line(aligned_sent, verb_form_dict)
...
def shuffle_news_data(NEWS_FILE_NAME_SPLIT,NEWS_FILE_NAME_SPLIT_SHUFFLED):
    """Generate some news data"""
    print("Reading Spilt Data")
    org_answers = open(NEWS_FILE_NAME_SPLIT, encoding='utf-8').read().split("\n")
    if len(org_answers)<1:
        print('the file NEWS_FILE_NAME_SPLIT is corrupted!')
    else:
        print('shuffle', end=" ")
        random_shuffle(org_answers)
        print("Done")
        with open(NEWS_FILE_NAME_SPLIT_SHUFFLED, "w",encoding='utf-8') as output_file:
            output_file.write("\n".join(org_answers))
...
def get_target_sent_by_levels(source_tokens, labels,encode_verb_form):
    ...
    for level in range(max_level):
        ...
        for edits in relevant_edits:
            ...
            if label == "$DELETE":
                ...
                word = apply_reverse_transformation(source_token, label,encode_verb_form)
                if word is None:
                    word = source_token
                target_tokens[target_pos] = word
            elif label.startswith("$SUFFIXTRANSFORM"):
                word = apply_suffixtransform(source_token,  label)
                if word is None:
                    word = source_token
                target_tokens[target_pos] = word
...
def convert_tagged_line(line,encode_verb_form, delimeters=SEQ_DELIMETERS):
    ...
    levels_dict, target_line = get_target_sent_by_levels(source_tokens, labels,encode_verb_form)
    return target_line
...
def main(args):
    ...
    convert_data_from_raw_files(args.source, args.target, args.output_file, args.chunk_size,verb_form_dict)
    #shuffle_news_data(args.output_file,args.output_file_shuffled)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chunk_size',
                        ...
                        default=1000000)
    parser.add_argument('-osh', '--output_file_shuffled',
                        help='Path to the output file')
