...
from utils.transform_suffixes_new import apply_suffixtransform
...
def get_verb_form_dicts(path_to_dict):
    #path_to_dict = "../resources/verb-form-vocab.txt"
    encode, decode = {}, {}
...
def local_tokenizer(doc,nlp):
    #doc = ''.join(x.rstrip('\n') for x in doc)
    doc = sent_tokenize(doc)
    tok_doc = []
    for sent in doc:
        sent = nlp.tokenizer(sent)
        tokens = [token.text for token in sent]
        sent = " ".join(tokens) + "\n"
        tok_doc.append(sent)
    return tok_doc

def get_target_sent_by_edits(source_tokens, edits,encode_verb_form):
    target_tokens = source_tokens[:]
    shift_idx = 0
    for edit in edits:
        start, end, label, _ , _, _ = edit
...
"""
def convert_using_verb(token, smart_action,encode_verb_form):
    key_word = "$TRANSFORM_VERB_"
    if not smart_action.startswith(key_word):
        raise Exception(f"Unknown action type {smart_action}")
    encoding_part = f"{token}_{smart_action[len(key_word):]}"
    decoded_target_word = decode_verb_form(encoding_part,encode_verb_form)
    return decoded_target_word
...
def apply_reverse_transformation(source_token, transform,encode_verb_form):
    if transform.startswith("$TRANSFORM"):
        ...
        if transform.startswith("$TRANSFORM_VERB"):
            return convert_using_verb(source_token, transform,encode_verb_form)
        # deal with split
...
def decode_verb_form(original,encode_verb_form):
    return encode_verb_form[1].get(original)


def encode_verb_form(original_word, corrected_word,encode_verb_form):
    decoding_request = original_word + "_" + corrected_word
    decoding_response = encode_verb_form[0].get(decoding_request, "").strip()
